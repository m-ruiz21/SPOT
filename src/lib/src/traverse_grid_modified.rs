...
#[pyfunction]
#[pyo3(text_signature = "(grid: list[list[float]], start: (int, int), end_y: int, moves: list[int, int], min_dist: float /)")]
pub fn traverse_grid(grid: Vec<Vec<f64>>, start: (i32, i32), end_y: usize, moves: Vec<(i32, i32)>, min_dist: f64) -> Vec<(usize, usize)> {
    ...
    while let Some((_, node)) = queue.pop() {
        ...
        for (move_idx, (dx, dy)) in moves.iter().enumerate() {
            ...
            if !valid_move(new_x, new_y, &grid, min_dist) {
                continue;
            } 
...
fn valid_move(x: i32, y: i32, grid: &Vec<Vec<f64>>, min_dist: f64) -> bool {
    ...
    return grid[y as usize][x as usize] > min_dist; 
}
